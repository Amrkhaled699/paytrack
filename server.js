...
function ensureOwnerAndMigrate(db){
  ...
  if(envOwners.length){
    ...
    throw new Error('Production startup requires PAYTRACK_OWNER_1_USERNAME/PASSWORD/TOTP_SECRET and PAYTRACK_OWNER_2_USERNAME/PASSWORD/TOTP_SECRET. No default Owner account is permitted in production.');
  } else {
...
function ownerMfaSecret(username){for(let i=1;i<=2;i++){if(cleanUsername(process.env[`PAYTRACK_OWNER_${i}_USERNAME`]||'')===cleanUsername(username))return process.env[`PAYTRACK_OWNER_${i}_TOTP_SECRET`]||'';}return '';}
